...
public class Main extends JFrame{
    ...
    private void setGameState(int gamestate, Main main)
    {
        ...
        if(gamestate == -1)
        {
            ...
            //Object gameover = new Object(gameFrame,5,5,GAMEOVER_PATH);
        }
        ...
        main.addComponentListener(new ComponentListener() {
            public void componentResized(ComponentEvent e) {
                ...
                gameFrame.player.loadImages();
                gameFrame.player.scaleImages();

                for(Enemy enemy : gameFrame.enemy_list) {
                    enemy.loadImages();
                    enemy.scaleImages();
                }
    ...
    public void loadMap(String code, GameFrame frame){
        ...
        for(int i=2; i<items.length; i++){
            ...
            switch (elements[2].charAt(0)) {
                case '1':
                    frame.addTile(X, Y, Tile.GRASS_PATH);
                    break;
                case '2':
                    frame.addTile(X, Y, Tile.GRASS_LIGHT_PATH);
                    break;
                case 'w':
